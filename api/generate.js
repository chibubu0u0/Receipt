...
- lyricEssence 可以呈現「最經典歌詞意象」。quote 欄位請優先填入非常短且完整的『歌曲原文歌詞片段』：英文歌請用英文，日文歌請用日文，韓文歌請用韓文，中文歌請用中文。quote 禁止翻譯成繁體中文；只有 summary 才用繁體中文整理。quote 限制：中文最多 10 個字，英文最多 5 個單字，日文/韓文等也請保持短句。不要截斷單字或半句；如果你完全不確定原文，quote 才填空字串。summary 必須用繁體中文、自己的話概括，不要複製歌詞。
- 如果你不確定冷門歌曲的完整資料，根據歌手風格、曲名語意與可推測的音樂情緒生成，但不要假裝知道不存在的細節。
- 只回傳 JSON，不要 markdown，不要解釋。
- 不要輸出你的思考過程、自我檢查過程或草稿。

生成前請在內部完成以下自我檢視，但不要把檢視文字輸出：
1. 先反思自己的第一版想法：這些情緒、色彩與物件是否只是通用模板？如果是，請改得更貼近這首歌。
2. 檢視整張收據是否合理：soul、emotions、colors、melody、objects、items、closing 之間要有一致的情緒邏輯。
3. 檢查產品感：最後輸出的內容要像一張可以被收藏、分享、截圖的「情感商品小票」，而不是散亂的音樂評論。
4. 檢查避免重複：不要重複使用上一首歌可能出現的固定詞彙、固定色票或固定物件。
5. 完成上述內部檢查後，只輸出修正後的最終 JSON。

本次深度規則：${depthRule}
用戶補充語氣：${note || "無"}

欄位寫作規則：
- lyricEssence：呈現「經典歌詞意象」。title 是 8 字以內小標；summary 是 1-2 句，用繁體中文、自己的話概括歌詞最被記住的情緒或畫面，不要照抄歌詞；quote 請優先填入完整『原文』短片段，英文歌用英文、日文歌用日文、韓文歌用韓文、中文歌用中文。中文最多 10 個字，英文最多 5 個單字。不能翻譯 quote，不能截斷單字或半句；只有完全不確定原文時才填空字串。
...
最終輸出前請再次內部確認：
- 情緒名稱不要互相矛盾。
- lyricEssence 必須是對歌詞意象的重新整理，不要變成逐字歌詞引用；quote 若有內容，必須是歌曲原始語言的短句，不要翻譯成中文。
- 色彩語言必須和歌曲氛圍有關，不要只是好看的顏色。
- 歌曲具現化物件要像真的能出現在畫面裡。
- 心情結帳項目的強度要和 emotions 的數值有呼應。
- 如果某個欄位看起來太普通，請先自行修正再輸出。
- quote 最終檢查：如果歌曲原文是英文，quote 不可以是中文；如果歌曲原文是日文/韓文，也要保留日文/韓文。`;
}
