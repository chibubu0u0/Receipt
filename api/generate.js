...
- lyricEssence 可以呈現「最經典歌詞意象」，但 quote 欄位只能放非常短且完整的『原文歌詞片段』：中文最多 10 個字，英文最多 5 個單字，日文/韓文等也請保持原文。不要翻譯 quote，不要為了符合限制而截斷單字或半句；如果無法完整呈現，quote 請填空字串。summary 必須用繁體中文、自己的話概括，不要複製歌詞。
...
欄位寫作規則：
- lyricEssence：呈現「經典歌詞意象」。title 是 8 字以內小標；summary 是 1-2 句，用繁體中文、自己的話概括歌詞最被記住的情緒或畫面，不要照抄歌詞；quote 只能是完整『原文』短片段，中文最多 10 個字，英文最多 5 個單字，日文/韓文等保留原文。不能翻譯 quote，不能截斷單字或半句；不確定就填空字串。
...
最終輸出前請再次內部確認：
- 情緒名稱不要互相矛盾。
- lyricEssence 必須是對歌詞意象的重新整理，不要變成逐字歌詞引用；quote 若有內容，必須是原文短句，不要翻譯。
